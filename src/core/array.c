...
Value* _ArrayEach(Interpreter* interpreter, int argc, Value** arguments) {
    ...
    int argNeeded = ValueIsNativeFunction(callback) ? CoerceToNativeFunction(callback)->Argc
                                                    : CoerceToUserFunction(callback)->Argc;

    if (argNeeded != 2 && argNeeded != VARARG) {
        return NewErrorValue(
            interpreter,
            "Callback function for Array.each must take exactly 2 arguments (item, index)");
    }
...
Value* _ArrayKeep(Interpreter* interpreter, int argc, Value** arguments) {
    ...
    if (argNeeded != 2 && argNeeded != VARARG) {
        return NewErrorValue(
            interpreter,
            "Callback function for Array.keep must take exactly 2 arguments (item, index)");
    }
