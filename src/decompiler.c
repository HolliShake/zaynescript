...
/**
 * @brief Copies the embedded NUL-terminated string stored in the bytecode image
 * at byte index alignStart.
 * @param codes Contiguous bytecode buffer; the string begins exactly at
 * codes+alignStart.
...
/**
 * @brief Decodes a big-endian 32-bit integer from the four bytes
 * codes[alignStart..alignStart+3].
...
/**
 * @brief Same as ValueToString() in value.c: human-readable text for any Value
 * tag, newly allocated.
 * @param value Runtime value being stringified for decompiler output.
 * @return New NUL-terminated heap string; each call site in this file frees()
 * once the text is appended.
 * @origin src/value.c
 */
...
String DecompileFunction(Interpreter* interpreter, UserFunction* uf) {
	...
	_AppendFmt(&result,
			   "Function: %s (argc: %d, locals: %d)\n",
			   uf->Name ? uf->Name : "<anonymous>",
			   uf->Argc,
			   uf->LocalC);
	...
	while (ip < uf->CodeC) {
		...
		switch (opcode) {
			...
			case OP_LOAD_INT:
				{
					int offset = ReadInt32(uf->Codes, ip);
					_AppendFmt(&result, "OP_LOAD_INT %d\n", offset);
					ip += 4;
					break;
				}
			case OP_LOAD_BOOL:
			...
			case OP_ARRAY_EXTEND:
				_Append(&result, "OP_ARRAY_EXTEND\n");
				break;
			case OP_ARRAY_PUSH:
				_Append(&result, "OP_ARRAY_PUSH\n");
				break;
			case OP_ARRAY_MAKE:
				{
			...
			case OP_OBJECT_EXTEND:
				_Append(&result, "OP_OBJECT_EXTEND\n");
				break;
			case OP_OBJECT_PLUCK_ATTRIBUTE:
				{
					String str = ReadString(uf->Codes, ip);
					_AppendFmt(&result,
							   "OP_OBJECT_PLUCK_ATTRIBUTE \"%s\"\n",
							   str);
					ip += strlen(str) + 1;
					free(str);
					break;
			...
			case OP_CLASS_EXTEND:
				_Append(&result, "OP_CLASS_EXTEND\n");
				break;
			case OP_CLASS_MAKE:
				{
					String str = ReadString(uf->Codes, ip);
			...
			case OP_CLASS_DEFINE_STATIC_MEMBER:
				_Append(&result, "OP_CLASS_DEFINE_STATIC_MEMBER\n");
				break;
			case OP_CLASS_DEFINE_INSTANCE_MEMBER:
				_Append(&result, "OP_CLASS_DEFINE_INSTANCE_MEMBER\n");
				break;
			case OP_SET_INDEX:
				_Append(&result, "OP_SET_INDEX\n");
				break;
			case OP_GET_INDEX:
				_Append(&result, "OP_GET_INDEX\n");
				break;
			...
			case OP_NOT:
				_Append(&result, "OP_NOT\n");
				break;
			case OP_POS:
				_Append(&result, "OP_POS\n");
				break;
			case OP_NEG:
			...
			case OP_MUL:
				...
				break;
			case OP_DIV:
				_Append(&result, "OP_DIV\n");
				break;
			case OP_MOD:
				_Append(&result, "OP_MOD\n");
				break;
			case OP_INC:
				_Append(&result, "OP_INC\n");
				break;
			case OP_POSTINC:
				_Append(&result, "OP_POSTINC\n");
				break;
			case OP_ADD:
				_Append(&result, "OP_ADD\n");
				break;
			case OP_DEC:
				_Append(&result, "OP_DEC\n");
				break;
			case OP_POSTDEC:
				_Append(&result, "OP_POSTDEC\n");
				break;
			case OP_SUB:
				_Append(&result, "OP_SUB\n");
				break;
			case OP_LSHFT:
				_Append(&result, "OP_LSHFT\n");
				break;
			case OP_RSHFT:
				_Append(&result, "OP_RSHFT\n");
				break;
			case OP_LT:
				_Append(&result, "OP_LT\n");
				break;
			case OP_LTE:
				_Append(&result, "OP_LTE\n");
				break;
			case OP_GT:
				_Append(&result, "OP_GT\n");
				break;
			case OP_GTE:
				_Append(&result, "OP_GTE\n");
				break;
			case OP_EQ:
				_Append(&result, "OP_EQ\n");
				break;
			case OP_NE:
				_Append(&result, "OP_NE\n");
				break;
			case OP_AND:
				_Append(&result, "OP_AND\n");
				break;
			case OP_OR:
				_Append(&result, "OP_OR\n");
				break;
			case OP_XOR:
				_Append(&result, "OP_XOR\n");
				break;
			...
			case OP_LOCK_VAR:
				{
					int offset = ReadInt32(uf->Codes, ip);
					_AppendFmt(&result, "OP_LOCK_VAR %d\n", offset);
					ip += 4;
					break;
				}
			case OP_DUPTOP:
				_Append(&result, "OP_DUPTOP\n");
				break;
			case OP_DUP2:
				_Append(&result, "OP_DUP2\n");
				break;
			case OP_POPTOP:
				_Append(&result, "OP_POPTOP\n");
				break;
			case OP_ROT2:
				_Append(&result, "OP_ROT2\n");
				break;
			case OP_ROT3:
				_Append(&result, "OP_ROT3\n");
				break;
			case OP_ROT4:
				_Append(&result, "OP_ROT4\n");
				break;
			case OP_SETUP_TRY:
				{
					int offset = ReadInt32(uf->Codes, ip);
			...
			case OP_RAISE:
				_Append(&result, "OP_RAISE\n");
				break;
			case OP_RETURN:
				_Append(&result, "OP_RETURN\n");
				break;
			default:
