...
#define LINE_NUM_WIDTH 4
	...
	for (int line = startLine; line <= endLine; line++) {
		...
		int lineEndIdx =
			(line < lineCount) ? lineStarts[line] - 1 : lineStartIdx;
		...
		for (int i = lineStartIdx;
			 i <= actualLineEnd && runes[i] != '\n' && runes[i] != 0;
			 i++) {
			unsigned char utf8Buffer[5];
			int			  utf8Size = utf_encode_char(runes[i], utf8Buffer);

			for (int j = 0; j < utf8Size && bufferIdx < MAX_LINE_WIDTH - 5;
				 j++) {
...
String NormalizePath(String pathStr) {
	...
	String result =
		Allocate(strlen(pathStr) + 2);	// +2 for potential root slash and null
	result[0] = '\0';
