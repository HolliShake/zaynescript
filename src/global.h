...
/**
 * @def VARARG
 * @brief Constant indicating variable number of arguments for
 * functions.
 */
#define VARARG -1
...
/**
 * @def PREC_DBL
 * @brief Precision for double-precision big-number operations
 * (53-bit mantissa).
 */
#define PREC_DBL 53
...
/**
 * @def PREC_NUM
 * @brief Computes the precision in bits for a big-number value
 * of given limb length.
 *
 ...
 *
 * Represents a single entry in a hash map bucket. Uses chaining
 * for collision resolution through the Next pointer.
 */
...
struct hashnode_struct {
	...
	HashNode* Next; /**< Pointer to the next node in the bucket
 ...
 * @brief Tracks the location of a token or AST node in source
 * code.
 *
 * Stores both line and column information for the start and end
 * positions of a token or syntax element. All values are
 * 1-based.
 */
 ...
 * @brief Enumeration of all possible token types in the
 * language.
 *
 ...
 *
 * Contains the token type, its string value, and its position in
 * the source.
 */
typedef struct token_struct {
	TokenKind Type;		/**< The type of the token */
	String	  Value;	/**< The string value of the token */
	Position  Position; /**< The location of the token in the
						   source code */
} Token;
 ...
 * @brief State machine for syntactic analysis of tokenized
 * source code.
 *
 * Consumes tokens from the lexer and builds an abstract syntax
 * tree.
 */
 ...
 *
 * Defines the complete set of syntax tree node types
 * representing all language constructs including statements,
...
typedef enum ast_type_enum {
	...
	AST_EXPRESSION_STATEMENT, /**< Statement wrapping an
								 expression */
	AST_IF,					  /**< If statement */
	...
	AST_SHORT_ASSIGN, /**< Short assignment (e.g. +=, -=) -
						 specific type determined by operation */
...
/**
 * @struct ast_struct
 * @brief Abstract Syntax Tree node representing parsed source
 * code structure.
 *
 * A flexible tree node structure that can represent any language
 * construct. The meaning of child nodes (A, B, C, D) depends on
 * the Type field. Nodes can be chained using the Next pointer
...
struct ast_struct {
	...
	String	 Value;	   /**< String value (for identifiers, literals,
						  etc.) */
 ...
 * @brief Enumeration of all possible value types in the
 * interpreter.
 *
 * Defines the runtime type system including primitives,
 * collections, functions, and objects.
 */
 ...
 *
 * Tagged union representing any runtime value. Uses a
 * discriminated union to store different value types
 * efficiently. Includes garbage collection metadata (Next
 * pointer and Marked flag).
 */
...
struct value_struct {
	...
	union value_union {
		...
		void*  Opaque; /**< Pointer to heap-allocated data
...
/**
 * @enum opcode_enum
 * @brief Enumeration of all possible opcode types for the
 * virtual machine.
 *
 * Defines the complete instruction set for the bytecode
 * interpreter. Instructions are organized by category: loading,
 * array/object operations, function operations, arithmetic,
...
typedef enum opcode_enum {
	...
	OP_LOCK_VAR,					 /**< Lock a variable for mutation in a
									closure */
 ...
 *
 * Describes how a variable from an outer scope is captured by a
 * closure, including the scope depth and the source/destination
 * indices.
 */
typedef struct capture_meta_struct {
	int Depth; /**< Depth of the captured variable's scope
 ...
 * @brief Represents a cell in the environment (variable
 * storage).
 *
 * A single variable slot in an environment. Tracks whether the
 * variable has been captured by a closure to enable proper
 * lifetime management.
 */
typedef struct envcell_struct {
	Value* Value;	   /**< Pointer to the value stored in the cell */
	bool   IsCaptured; /**< True if this cell is captured by a
						  closure */
	int RefCount;	   /**< Reference count for captured variables
						  (optional, can be used for optimizations) */
} EnvCell;
 ...
 *
 * Contains the local variables for a function or block scope,
 * along with a reference to the parent environment for lexical
 * scoping.
 */
 ...
 * @brief Stores source location information for debugging and
 * error reporting.
 *
 * Associates bytecode or AST nodes with their original source
 * file and line number, enabling meaningful error messages and
 * stack traces.
 */
 ...
 *
 * Contains the compiled bytecode, parameter information, local
 * variable count, and closure captures for a function defined in
 * the source language.
 */
typedef struct user_function_struct {
	Value* Scope;			   /**< The environment where the function was
								  defined */
 ...
 *
 * Tracks information about a variable or constant during
 * compilation, including its scope, mutability, and memory
 * location.
 */
typedef struct symbol_struct {
	bool IsGlobal;		 /**< True if symbol is global */
	bool IsLocalToFn;	 /**< True if symbol is local to current
							function */
	bool IsConstant;	 /**< True if symbol is a constant */
	int	 Offset;		 /**< Memory offset or index */
	int	 ReferenceCount; /**< Reference count for optimizations
						   (optional) */
} Symbol;
 ...
 *
 * Defines the different kinds of scopes that can exist during
 * compilation, which affects variable resolution and control
 * flow handling.
 */
 ...
 *
 * Maintains symbol tables and control flow information during
 * compilation. Scopes are nested and linked through the Parent
 * pointer. Loop scopes track jump locations for break and
 * continue statements.
 */
...
struct scope_struct {
	...
	// Loop
	int* ContinueJumps; /**< Array of jump locations for continue
						   statements */
	int	 ContinueJumpC; /**< Count of continue jumps */
	int* BreakJumps;	/**< Array of jump locations for break
						   statements */
 ...
 *
 * Contains the class name, optional base class for inheritance,
 * and separate hash maps for static and instance members.
 */
 ...
 *
 * Links to the class prototype and maintains instance-specific
 * member values.
 */
 ...
 *
 * Native functions are implemented in C and callable from the
 * interpreted language. They receive the interpreter context,
 * argument count, and argument array, and return a Value
 * pointer.
 *
 ...
 *
 * Wraps a C function pointer with metadata about the function's
 * name and expected argument count.
 */
 ...
 * @brief Represents a function or value exported by a native
 * module.
 *
 * Native modules can export both functions and constant values.
 * This structure describes a single export with its name and
 * either a function pointer or a value pointer.
 */
typedef struct module_function_struct {
	const String		   Name;	  /**< Export name */
	int					   Argc;	  /**< Argument count (for functions) */
	NativeFunctionCallback CFunction; /**< C function pointer (if is
 ...
 * @brief Represents a single entry in the call stack for
 * debugging.
 *
 * Stores the line information and function value for a
 * particular call frame.
 */
typedef struct stack_trace {
	LineInfo line;	   /**< Source location at this call frame */
	Value*	 Function; /**< Function value active at this call
						  frame */
} StackTrace;

/**
 * @struct import_node
 * @brief Represents a single node in the module import linked
 * list.
 *
 ...
 * @brief Import cycle-detection state: module not yet visited
 * during loading.
 */
...
/**
 * @def VISITING
 * @brief Import cycle-detection state: module is currently being
 * loaded (cycle risk).
 */
#define VISITING 1
...
/**
 * @def SAFE
 * @brief Import cycle-detection state: module has been fully
 * loaded with no cycle.
 */
#define SAFE 2
...
typedef struct import_node {
	...
	int State;				   /**< Flag for cycle detection during module
								  loading */
	ImportNode* Next;		   /**< Pointer to the next import node in the
								  linked list */
} ImportNode;

/**
 * @struct exception_handler_struct
 * @brief Stores state required to resume execution after an
 * exception jump.
 *
 * Used by the interpreter's exception handling stack to track
 * where execution should continue and which instruction pointer
 * was paused.
 */
 ...
 * @brief Compiler state structure holding parser and interpreter
 * references.
 *
 * Links the compilation process to both the parser (for reading
 * source code) and the interpreter (for registering constants
 * and functions).
 */
...
/**
 * @def Allocate
 * @brief Wrapper macro for memory allocation with file/line
 * tracking.
 *
 * Provides debugging information by recording the source
 * location of each allocation.
 *
...
/**
 * @def Callocate
 * @brief Wrapper macro for zero-initialized memory allocation
 * with tracking.
 *
 * Allocates memory and initializes it to zero, with debugging
 * information.
 *
 ...
 *
 * Allocates memory and tracks the allocation location for
 * debugging. Terminates the program if allocation fails.
 *
 ...
 *
 * Allocates memory, initializes it to zero, and tracks the
 * allocation location for debugging. Terminates the program if
 * allocation fails.
 *
 * @param file Source file name calling allocation.
 * @param line Line number calling allocation.
 ...
 *
 * Resizes an existing memory block and tracks the allocation
 * location for debugging. Terminates the program if reallocation
 * fails.
 *
 * @param file Source file name calling allocation.
 * @param line Line number calling allocation.
 ...
 *
 * Creates a new copy of the given string on the heap using the
 ...
 *
 * Decodes a UTF-8 encoded string into an array of Unicode code
 * points. The returned array is null-terminated.
 *
 ...
 *
 * Converts a runtime value to an integer, performing type
 * conversion as necessary. Behavior depends on the value's type.
 *
 ...
 *
 * Converts a runtime value to a long integer, performing type
 * conversion as necessary. Behavior depends on the value's type.
 *
 * @param value The value to coerce.
 ...
 *
 * Converts a runtime value to a double, performing type
 * conversion as necessary. Behavior depends on the value's type.
 *
 * @param value The value to coerce.
 ...
 *
 * Converts a runtime value to a big integer, performing type
 * conversion as necessary. Behavior depends on the value's type.
 * Uses the libbf library for big integer representation.
 *
...
/**
 * @brief Gets the precession (number of decimal places) for a
 * big numeric value.
 *
 ...
 *
 * Converts a runtime value to a boolean, following standard
 * truthiness rules. Behavior depends on the value's type.
 *
 * @param value The value to coerce.
 ...
 *
 * Extracts the Environment pointer from a value. The value must
 * be of type VLT_ENVIRONMENT.
 *
 * @param value The value to coerce.
 ...
 *
 * Extracts the UserFunction pointer from a value. The value must
 * be of type VLT_USER_FUNCTION.
 *
 * @param value The value to coerce.
 ...
 *
 * Extracts the NativeFunction pointer from a value. The value
 * must be of type VLT_NATV_FUNCTION.
 *
 * @param value The value to coerce.
 ...
 *
 * Extracts the ClassInstance pointer from a value. The value
 * must be of type VLT_CLASS_INSTANCE.
 *
 * @param value The value to coerce.
 ...
 *
 * Creates a formatted error message that includes the source
 * file path, line number, column number, and a snippet of the
 ...
 *
 * Prints a formatted error message with source location
 ...
 *
 * Similar to sprintf but allocates a new string to hold the
 * result. Uses the tracked allocation system.
 *
 ...
 *
 * Uses the libbf library to convert a big integer (bf_t) to its
 * string representation. The returned string is allocated using
 * the tracked allocation system.
 *
 ...
 *
 * Uses the libbf library to convert a big numeric (bf_t) to its
 * string representation. The returned string is allocated using
 * the tracked allocation system.
 *
 ...
 * separator, removes redundant separators, and resolves any "."
 * or ".." components. The resulting path is in a consistent
 ...
 *
 * Extracts the file name component from a file path. If the path
 * ends with a directory separator, an empty string is returned.
 *
 * @param pathStr The input path string.
 ...
 *
 * Extracts the directory component from a file path. If the path
 * does not contain a directory separator, an empty string is
 * returned.
 *
 * @param pathStr The input path string.
...
/**
 * @brief Converts a relative path to an absolute path based on a
 * given base path.
 *
 * If the input path is already absolute, it is returned as-is.
 * Otherwise, it is combined with the base path to produce an
 * absolute path. The resulting path is normalized to remove any
 * redundant components (e.g. "." or "..").
 *
 * @param baseStr The base path to resolve against (must be an
 * absolute path).
...
/**
 * @brief Converts a relative path to an absolute path based on
 * the current working directory.
 *
 * If the input path is already absolute, it is returned as-is.
 * Otherwise, it is combined with the current working directory
 * to produce an absolute path. The resulting path is normalized
