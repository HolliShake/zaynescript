...
UserFunction* UserFunctionClone(UserFunction* userFunction) {
    UserFunction* clone =
        CreateUserFunction(userFunction->Name != NULL ? AllocateString(userFunction->Name) : NULL,
                           userFunction->Argc,
                           userFunction->Async);
    clone->Scope = userFunction->Scope;
    clone->LineC = userFunction->LineC;
    clone->Lines = Reallocate(clone->Lines, sizeof(LineInfo) * (userFunction->LineC + 1));
    memcpy(clone->Lines, userFunction->Lines, sizeof(LineInfo) * (userFunction->LineC + 1));
    clone->CodeC = userFunction->CodeC;
    clone->Codes = Reallocate(clone->Codes, sizeof(uint8_t) * (userFunction->CodeC + 1));
    ...
    clone->CaptureMetas =
        Reallocate(clone->CaptureMetas, sizeof(CaptureMeta) * (userFunction->CaptureC + 1));
    memcpy(clone->CaptureMetas,
           userFunction->CaptureMetas,
           sizeof(CaptureMeta) * userFunction->CaptureC);
    clone->Captures = Reallocate(clone->Captures, sizeof(EnvCell*) * (userFunction->CaptureC + 1));
...
int UserFunctionAddCapture(UserFunction* userFunction, int depth, int sourceOffset) {
    ...
    userFunction->CaptureMetas =
        Reallocate(userFunction->CaptureMetas, sizeof(CaptureMeta) * (userFunction->CaptureC + 1));
    userFunction->Captures[offset] = NULL;
    userFunction->Captures =
        Reallocate(userFunction->Captures, sizeof(EnvCell*) * (userFunction->CaptureC + 1));
    return offset;
}
...
String UserFunctionToString(UserFunction* userFunction) {
    ...
    if (userFunction->Argc == -1) {
        ...
        size_t nameLen = strlen(name);
        size_t bufferSize =
            nameLen + argsSize + 32;  // "function " + name + "(" + args + ") {...}" + null
        String buffer = Allocate(bufferSize);
...
void FreeUserFunction(UserFunction* userFunction) {
    ...
    if (userFunction->Name != NULL)
        free(userFunction->Name);
...
String NativeFunctionMetaToString(NativeFunction* meta) {
    ...
    if (meta->Argc == -1) {
        ...
        size_t nameLen = strlen(meta->Name);
        size_t bufferSize =
            nameLen + argsSize + 32;  // "native function " + name + "(" + args + ") {...}" + null
        String buffer = Allocate(bufferSize);
...
void FreeNativeFunction(NativeFunction* nativeFunction) {
    if (nativeFunction->Name != NULL)
        free(nativeFunction->Name);
    free(nativeFunction);
}