		...
		case TK_KEY:
			{
				...
				if (strcmp(key, KEY_TRUE) == 0 || strcmp(key, KEY_FALSE) == 0) {
					node = AstBool(strcmp(parser->Next.Value, KEY_TRUE) == 0,
								   parser->Next.Position);
					ACCEPTT(TK_KEY);
					free(key);
...
static Ast* _ParseAugmentedAddetive(Parser* parser) {
	...
	while (CHECKTV("+=") || CHECKTV("-=")) {
		...
		lhs =
			AstBinary((strcmp(op, "+=") == 0 ? AST_ADD_ASSIGN : AST_SUB_ASSIGN),
					  lhs,
					  rhs,
					  MergePositions(lhs->Position, rhs->Position));
...
static Ast* _ParseAugmentedShift(Parser* parser) {
	...
	while (CHECKTV("<<=") || CHECKTV(">>=")) {
		...
		lhs = AstBinary(
			(strcmp(op, "<<=") == 0 ? AST_LSHFT_ASSIGN : AST_RSHFT_ASSIGN),
			lhs,
			rhs,
			MergePositions(lhs->Position, rhs->Position));
...
static Ast* _ParseInitializerConditionMutator(Parser* parser) {
	...
	while (CHECKTV(":=")) {
		...
		lhs = AstBinary(AST_SHORT_ASSIGN,
						lhs,
						rhs,
						MergePositions(lhs->Position, rhs->Position));
...
static Ast* _ParseVarStatement(Parser* parser) {
	...
	ended			  = parser->Next.Position;
	ACCEPTV_FREE(";");
	return AstVarDeclaration(AST_VAR_DECLARATION,
							 declarations,
							 MergePositions(start, ended));
}
...
static Ast* _ParseConstStatement(Parser* parser) {
	...
	ended			  = parser->Next.Position;
	ACCEPTV_FREE(";");
	return AstVarDeclaration(AST_CONST_DECLARATION,
							 declarations,
							 MergePositions(start, ended));
}
...
static Ast* _ParseLocalStatement(Parser* parser) {
	...
	ended			  = parser->Next.Position;
	ACCEPTV_FREE(";");
	return AstVarDeclaration(AST_LOCAL_DECLARATION,
							 declarations,
							 MergePositions(start, ended));
}
...
static Ast* _ParseIfStatement(Parser* parser) {
	...
	return AstIf(initializerCondition,
				 thenBranch,
				 elseBranch,
				 MergePositions(start, ended));
}
...
static Ast* _ParseForStatement(Parser* parser) {
	...
	ACCEPTV_FREE(";");
	if (initializerConditionMutator != NULL
		&& initializerConditionMutator->Next != NULL) {
	...
	ended = body->Position;
	return AstFor(initializerConditionMutator,
				  body,
				  MergePositions(start, ended));
}
...
static Ast* _ParseWhileStatement(Parser* parser) {
	...
	ended = body->Position;
	return AstWhile(initializerConditionMutator,
					body,
					MergePositions(start, ended));
}
...
static Ast* _ParseProgram(Parser* parser) {
	Ast* ast =
		AstProgram(_ParseListOfStatements(parser), parser->Next.Position);
